...
<html lang="en">
    ...
    <div class="page-wrapper">
                ...
                <ul class="side-menu metismenu">
                    ...
                    <li class="active">
                        ...
                        <ul class="nav-2-level collapse in">
                            ...
                            <li>
                                <a href="updateStore.jsp">Cập nhật thông tin</a>
                            </li>
                        </ul>
                    </li>
                </ul>
        ...
        <div class="gg" style="">
            <div class="row"
                 style="border: 0px ; border-radius: 5px; width: 1600px; margin: 0 auto; padding: 10px;">
                <div class="col-sm-6" style="height: 150%;">
                    <h2 class="myclass">Cập Nhập Sản Phẩm</h2>
                    <form action="ProductUpdateServlet?action=${ACTION}" method="post" enctype="multipart/form-data">

                        <div class="form-group">
                            <label>Mã Sản Phẩm</label>
                            <input type="text" value="${INFOR.productId}" class="form-control" name="productId" placeholder="Enter Product ID" readonly="">
                        </div>

                        <div class="form-group">
                            <label>Mã Số Cửa Hàng</label>
                            <input type="text" value="${INFOR.storeId}" class="form-control" name="storeId" placeholder="Enter Store ID" readonly="">
                        </div>

                        <div class="form-group">
                            <label>Tên Sản Phẩm</label>
                            <input type="text" value="${INFOR.productName}" class="form-control" name="productName" placeholder="Enter Product Name" style="height: 40px;">
                        </div>



                        <div class="form-group">
                            <label>Thông Tin Chi Tiết Của Sản Phẩm</label>
                            <textarea value="${INFOR.description}" class="form-control" name="description" placeholder="Enter Description" style="height: 415px;">${INFOR.description}</textarea>
                        </div>


                        <div class="form-group">
                            <label>Giá Bán Của Sản Phẩm</label>
                            <input type="number" value="${INFOR.price}" step="0.0000000001" min="0"
                                   class="form-control" name="price" placeholder="Enter Price" onkeydown="return event.keyCode !== 189;">
                        </div>
                ...
                <div class="col-sm-6">
                    ...
                    <div class="form-group" style="margin-top: 16px">
                        <label>Loại Của Sản Phẩm</label>
                        <select class="form-control" name="categoryName" required>
                            <option value="Bird" ${INFOR.categoryName == 'Bird' ? 'selected' : ''}>Chim</option>
                            <option value="Food" ${INFOR.categoryName == 'Food' ? 'selected' : ''}>Thức ăn</option>
                            <option value="Bird cage" ${INFOR.categoryName == 'Bird cage' ? 'selected' : ''}>Lồng Chim</option>
                            <option value="Accessory" ${INFOR.categoryName == 'Accessory' ? 'selected' : ''}>Phụ kiện</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label>Trạng Thái Của Sản Phẩm</label>
                        <select class="form-control" name="status" required>
                            <option value="Available" ${INFOR.status == 'Available' ? 'selected' : ''}>Còn Hàng</option>
                            <option value="Out of stock" ${INFOR.status == 'Out of stock' ? 'selected' : ''}>Hết Hàng</option>
                            <!--<option value="Out of stock" ${INFOR.status == 'Hidden' ? 'selected' : ''}>Hidden</option>-->
                        </select>
                    </div>

                    <div class="form-group">
                        <label>Số Lượng Của Sản Phẩm</label>
                        <input type="number" value="${INFOR.quantity}" 
                               class="form-control" name="quantity" min="1" step="1"
                               placeholder="Enter Quantity" pattern="[0-9]+" required onkeydown="return event.keyCode !== 189;">
                    </div>
